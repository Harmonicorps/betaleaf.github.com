...
For Each f1 in fc
	if (LCase(Right(f1.name,4)) = ".rdp") then
		if not GetRDPvalue(f1,"remoteapplicationname:s:") = "" then
			...
			Response.write "<Resource ID=""" & appresourceid & """ Alias=""" & appalias & """ Title=""" & apptitle & """ LastUpdated=""" & filedatetime & """ Type=""RemoteApp"">" & vbCRLF
			Response.write "<Icons>" & vbCRLF
			Response.write "<IconRaw FileType=""Ico"" FileURL=""/icon/" & appicon & """ />" & vbCRLF
			Response.write "</Icons>" & vbCRLF
			if appftastring <> "" then
				Response.write "<FileExtensions>" & vbCRLF
				appftaarray = Split(appftastring,",")
				for each filetype in appftaarray
					docicon = Left(f1.name,Len(f1.name)-4) & filetype & ".ico"
					Response.write "<FileExtension Name=""" & filetype & """ PrimaryHandler=""True"">" & vbCRLF
					Response.write "<FileAssociationIcons>" & vbCRLF
					Response.write "<IconRaw FileType=""Ico"" FileURL=""/icon/" & docicon & """ />" & vbCRLF
					Response.write "</FileAssociationIcons>" & vbCRLF
			...
			Response.write "<ResourceFile FileExtension="".rdp"" URL=""/rdp/" & apprdpfile & """ />" & vbCRLF

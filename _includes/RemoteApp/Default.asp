...
For Each f1 in fc
	if (LCase(Right(f1.name,4)) = ".rdp") then
		if not GetRDPvalue(f1,"remoteapplicationname:s:") = "" then
			pngname = Left(f1.name,Len(f1.name)-4) & ".png"
			set pngfs = CreateObject("Scripting.FileSystemObject")
			scriptloc = Replace(Request.ServerVariables("PATH_TRANSLATED"),Replace(Request.ServerVariables("SCRIPT_NAME"),"/",""),"")
			If pngfs.FileExists(scriptloc & "png\" & pngname) Then
				pngpath = "png/" & pngname
			Else			
				pngpath = "rdpicon.png"
			End If	
			Response.write "<div id=apptile>"
			Response.write "<a href=""" & "rdp/" & f1.name & """><img border=0 height=64 width=64 src=""" & pngpath & """><br>" & GetRDPvalue(f1,"remoteapplicationname:s:") & "</a>"
			Response.write "</div>"
		end if
	end if
Next
